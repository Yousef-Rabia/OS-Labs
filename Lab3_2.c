...
int Calc(int start,int end,int p,int* Mid,int* Final){
...
int main(int argc, char *argv[])
{
    ...
    int* Mid,*Final; //Total Grades (mid + final) array
    ...
    Mid=malloc(S*sizeof(int));
    Final=malloc(S*sizeof(int));
    int k=0;
    while (!feof(grades))
    {
        int mid,final;
    ...
    for (int i=0; i<N; i++)
    {
        ...
        if (pid==0) 
        { // Child
           int Res=Calc(st,end,P,Mid,Final); 
           exit(Res);
        }
        else {
            //Parent
